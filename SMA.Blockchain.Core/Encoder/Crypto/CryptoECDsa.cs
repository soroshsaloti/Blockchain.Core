...
public static class CryptoECDsa
{
    public static KeyPair GenerateKeyPair()
    {
        ...
        using (ECDsa dsa = ECDsa.Create(ECCurve.NamedCurves.nistP256))
        {
            ...
            return new KeyPair(Params.Q.X.Concat(Params.Q.Y).ToArray(), Params.D);
        }
    }
    ...
    public static string Sign(KeyPair KeyPair, string data)
    {
        if (!KeyPair.HasPrivateKey())
        {
            throw new MissingFieldException("Missing private key.");
        }
        return Sign(KeyPair.PrivateKey, Encoding.UTF8.GetBytes(data)).ToHex();
    }
    ...
    public static bool Verify(KeyPair KeyPair, string signatureHex, string Message)
    {
        if (!KeyPair.HasPublicKey())
        {
            throw new MissingFieldException("Missing public key.");
        }
        return Verify(KeyPair.GetPublicKey(), signatureHex.FromHex(), Encoding.UTF8.GetBytes(Message));
    }
}
